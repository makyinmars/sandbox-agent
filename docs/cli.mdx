...
```bash
sandbox-agent api sessions update <SESSION_ID> [OPTIONS]
```
...
```bash
sandbox-agent api sessions update my-session \
  --model claude-3-5-sonnet-20240620
```

Notes:
1. Claude/Amp model changes are only allowed before the session has started.
2. Variant updates are only supported for OpenCode sessions.
...
| `api sessions update` | `PATCH /v1/sessions/{sessionId}` |
